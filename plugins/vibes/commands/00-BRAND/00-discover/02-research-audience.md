...
Some requests here are bigger than one pass: audience research that draws on several sources, such as interviews, reviews, forums, and competitor customers. When the work involves several steps or sources, paths that can run in parallel, checks, real risk, or approvals, use AskUserQuestion to offer designing it first with the `claude-vibes:graph-engineering` skill. The skill decides whether a graph is warranted, and it stops at the user's approval. To carry out an approved graph, follow the `claude-vibes:graph-running` skill, using the brand-audience-researcher agent for the research lanes and a separate agent as the skeptic. For a simple request, skip this and carry on below.
...
- **Structured reasoning**: Structure your analysis through the Four Forces and Three Job Types systematically
