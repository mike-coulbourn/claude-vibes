...
**Use the Agent tool to launch subagents for the checks.** Do not run the checks yourself. Launch them as parallel subagents for efficiency.
...
**Use the Agent tool to launch subagents in parallel** for each applicable check, focusing only on changed files. Use `subagent_type: "general-purpose"` for each check:
