# Competitive visual audit templates

Structured templates for capturing and applying competitive brand insights.

---

## Color audit matrix

```
| Competitor | Primary Color | Hex | Secondary Colors | Dominant Mood |
|------------|---------------|-----|------------------|---------------|
| Competitor A | Blue | #003366 | White, Gray | Professional, Corporate |
| Competitor B | Green | #00AA55 | White, Black | Fresh, Eco |
| Competitor C | ________ | ________ | ________ | ________ |
| Competitor D | ________ | ________ | ________ | ________ |
| Competitor E | ________ | ________ | ________ | ________ |

**Color Clusters Identified**: _________________________________
(Where do competitors congregate? e.g., "Most cluster around blue/teal")

**Color White Space**: _________________________________
(What color territories are unclaimed? e.g., "Purple, orange, warm tones")

**Strategic Implication**: _________________________________
(What does this mean for your brand's color selection?)
```

---

## Typography audit matrix

```
| Competitor | Primary Typeface | Style | Weight Usage | Personality |
|------------|------------------|-------|--------------|-------------|
| Competitor A | Helvetica | Sans-serif | Regular, Bold | Neutral, Clean |
| Competitor B | Playfair Display | Serif | Light, Bold | Elegant, Traditional |
| Competitor C | ________ | ________ | ________ | ________ |
| Competitor D | ________ | ________ | ________ | ________ |
| Competitor E | ________ | ________ | ________ | ________ |

**Typography Patterns**: _________________________________
(What dominates? e.g., "Geometric sans-serifs dominate")

**Typography White Space**: _________________________________
(What approaches are missing? e.g., "No humanist sans, no display fonts")

**Typography Signals Reference**:
- High contrast fonts (Bodoni): sophistication, luxury
- Low contrast fonts (Helvetica): sturdiness, trust
- Geometric sans (Futura): modern, technical
- Humanist sans (Gill Sans): approachable, friendly
- Serif fonts: traditional, established, trustworthy
- Display fonts: personality, distinctiveness
```

---

## Imagery style audit

```
| Competitor | Photo vs. Illust. | Subjects | Mood | Quality |
|------------|-------------------|----------|------|---------|
| Competitor A | Stock photography | People, products | Corporate | Medium |
| Competitor B | Custom illustration | Abstract, icons | Playful | High |
| Competitor C | ________ | ________ | ________ | ________ |
| Competitor D | ________ | ________ | ________ | ________ |
| Competitor E | ________ | ________ | ________ | ________ |

**Imagery Patterns**: _________________________________
(What dominates? e.g., "Stock photography with smiling people")

**Imagery White Space**: _________________________________
(What approaches are missing? e.g., "No custom illustration, no abstract")

**Style Considerations**:
- Photography: More credibility, higher cost, harder to scale globally
- Illustration: Unlimited flexibility, unique differentiation, easier to scale
- Mixed: Combine strategically with clear rules
```

---

## Perceptual map template

```
                        [Axis 2 High Label]
                              │
                              │
        Competitor A          │          Competitor B
                              │
                              │
[Axis 1 Low] ─────────────────┼───────────────── [Axis 1 High]
                              │
                              │
        Competitor C          │          Competitor D
                              │
                              │
                        [Axis 2 Low Label]

AXES DEFINITION:
Axis 1: _________________ ←→ _________________
        (e.g., Affordable)     (e.g., Premium)

Axis 2: _________________ ←→ _________________
        (e.g., Traditional)    (e.g., Innovative)

WHITE SPACE QUADRANT: _________________________________
(Which quadrant has least competition?)

POSITIONING OPPORTUNITY: _________________________________
(What positioning could this brand own?)
```

**Common Axis Pairs**:
- Price (Low → High) vs. Quality (Low → High)
- Professional ↔ Friendly
- Traditional ↔ Innovative
- Simple ↔ Powerful
- For Experts ↔ For Everyone
- Local ↔ Global
- Established ↔ Challenger

---

## Good/Different Chart template

```
                    DIFFERENT (10)
                         │
                         │
      [Competitor]       │     [Competitor]
                         │
                         │
         ────────────────┼──────────────── GOOD (10)
                         │
      [Competitor]       │     [Competitor]
                         │
                         │

COMPETITOR SCORES:

| Competitor | Good (1-10) | Different (1-10) | Quadrant |
|------------|-------------|------------------|----------|
| A | ___ | ___ | _________________ |
| B | ___ | ___ | _________________ |
| C | ___ | ___ | _________________ |
| D | ___ | ___ | _________________ |

QUADRANT KEY:
- Zone of Dominance (upper right): Good AND Different, the ideal position
- Zone of Commoditization (lower right): Good but not Different, which leads to price wars
- Zone of Irrelevance (upper left): Different but not Good, gimmicks
- Zone of Mediocrity (lower left): Neither, struggling

STRATEGIC IMPLICATION: _________________________________
(What does this analysis reveal about opportunity?)
```

---

## Individual competitor profile template

```
### [Competitor Name]

**Overview**
- Website: [URL]
- Positioning: [How they position themselves]
- Target Audience: [Who they're targeting]

**Visual Identity**
- Logo Style: [Wordmark/symbol/combination, style description]
- Primary Colors: [Colors + hex values]
- Secondary Colors: [Supporting palette]
- Typography: [Modern/traditional, serif/sans-serif, weight]
- Imagery: [Photography/illustration, subjects, mood]
- Overall Aesthetic: [Minimal/bold/playful/corporate/etc.]

**Brand Voice**
- Tone: [Professional/casual/playful/authoritative]
- Personality: [3-4 adjectives]
- Sample Language: "[Example phrase from their site]"

**Positioning Assessment**
- Tagline: [If they have one]
- Key Claims: [What they claim to be/do]
- Differentiation Angle: [How they try to stand out]

**Only-ness Assessment**
Can they complete: "Our brand is the only ___ that ___ for ___ who ___"?
- [ ] Yes, clearly differentiated
- [ ] Partially, generic positioning
- [ ] No, undifferentiated

**Good/Different Scores**
- Good: [1-10]. Rationale: _______________
- Different: [1-10]. Rationale: _______________
- Quadrant: [Dominance/Commoditization/Irrelevance/Mediocrity]

**User Perception**
- What Users Love: [From reviews/comments]
- What Users Criticize: [From reviews/comments]
```

---

## Zig vs zag decision matrix

```
ZIG FACTORS (Reasons to follow conventions):
□ Category requires trust/safety signals (healthcare, finance, legal)
□ Customers use visual conventions to identify legitimate options
□ Entering established market needing initial credibility
□ Differentiation comes from other factors (service, pricing, features)
□ Target audience values conformity/tradition

ZAG FACTORS (Reasons to break conventions):
□ Category is visually homogeneous (everyone looks the same)
□ Genuine philosophical/strategic difference to communicate
□ Target audience is tired of category sameness
□ Can sustain the difference with real substance
□ Willing to polarize some to attract others strongly
□ Brand philosophy challenges industry norms

SCORE:
Zig factors checked: ___
Zag factors checked: ___

RECOMMENDATION:
□ Zig (follow conventions) because: _______________
□ Zag (break conventions) because: _______________
□ Hybrid approach: zig on ___, zag on ___
```

---

## Output validation checklist

When reviewing competitive audit output, verify:

**Competitor Coverage**
- [ ] 5-10 competitors analyzed
- [ ] Direct and indirect competitors included
- [ ] Each has complete profile

**Visual Audit Matrices**
- [ ] Color audit matrix complete with hex values
- [ ] Typography audit matrix complete
- [ ] Imagery style audit complete
- [ ] Patterns and white space identified for each

**Strategic Analysis**
- [ ] Perceptual map with two relevant axes
- [ ] Competitors plotted on map
- [ ] White space identified
- [ ] Good/Different Chart with scores
- [ ] Quadrant analysis provided

**Differentiation Opportunities**
- [ ] Color opportunity identified
- [ ] Typography opportunity identified
- [ ] Imagery opportunity identified
- [ ] Zig vs zag recommendation with rationale

**Evidence Quality**
- [ ] Specific examples from competitor sites
- [ ] User perception data (from reviews)
- [ ] Visual descriptions are specific (not "modern logo")
- [ ] Hex codes for colors

**Strategic Output**
- [ ] Only-ness Statement opportunity provided
- [ ] Clear recommendations prioritized
- [ ] Risk assessment included
